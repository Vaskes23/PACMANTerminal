...
void CPrint::displayMap(WINDOW *win, const std::vector<std::vector<char> > &game_map,
                        ...
                        std::string game_tag, int cherrysEaten, int pointsEaten) {
    for (int i = 0; i < game_map.size(); ++i) {
        for (int j = 0; j < game_map[i].size(); ++j) {
            mvwaddch(win, i + 1, j + 1, displayed_map[i][j]);
        }
    }
    mvwprintw(win, 0, game_map[0].size() + 3, "Cherries eaten: %d", cherrysEaten);
    mvwprintw(win, 1, game_map[0].size() + 3, "Points eaten: %d", pointsEaten);
    mvwprintw(win, 2, game_map[0].size() + 3, "Game Tag: %s", game_tag.c_str());
    refresh();
    wrefresh(win);
}
